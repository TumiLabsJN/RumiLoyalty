...
```sql
-- Backend joins missions + mission_progress + rewards tables
SELECT
  ...
  -- From rewards table (reward data)
  ...
  "missions": [
    {
      ...
      // Reward data (from rewards table)
...
```typescript
// Extract reward values from rewards.value_data JSONB
function extractRewardValue(reward: Reward) {
  if (!reward.value_data) return null;

  const data = JSON.parse(reward.value_data);
