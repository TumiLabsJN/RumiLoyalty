...
**Reference:** See Loyalty.md Section 2 (Reward Names) and lines 1600-1606 (rewards table schema)

**Decision:** `rewards.name` field is auto-generated by backend from `type` + `value_data`/`description`. Admin cannot directly edit name field.
...
export const REWARD_DISPLAY_NAMES: Record<string, string> = {
...
/**
 * Generate reward name from type and data
 * @param reward - Reward object with type, value_data, description
 * @returns Auto-generated name string
 */
export function generateRewardName(reward: {
  type: string;
  value_data?: any;
  description?: string;
}): string {
  const baseName = REWARD_DISPLAY_NAMES[reward.type];
  ...
  if (reward.type === 'commission_boost') {
    const { percent, duration_days } = reward.value_data;
    const durationText = duration_days ? ` for ${duration_days} days` : '';
    return `${baseName}: +${percent}%${durationText}`;
  }

  if (reward.type === 'spark_ads') {
    const { amount } = reward.value_data;
  ...
  if (CATEGORY_2_TYPES.includes(reward.type)) {
    if (!reward.description) {
      throw new Error(`Category 2 reward '${reward.type}' requires description field`);
    }
    return `${baseName}: ${reward.description}`;
  }
...
```typescript
// Category 1 examples
generateRewardName({ type: 'commission_boost', value_data: { percent: 5, duration_days: 30 } })
// Returns: "Pay Boost: +5% for 30 days"

generateRewardName({ type: 'commission_boost', value_data: { percent: 7 } })
// Returns: "Pay Boost: +7%"

generateRewardName({ type: 'gift_card', value_data: { amount: 50 } })
// Returns: "Gift Card: $50"

generateRewardName({ type: 'spark_ads', value_data: { amount: 100 } })
// Returns: "Reach Boost: $100"

generateRewardName({ type: 'discount', value_data: { percent: 10, duration_days: 7 } })
// Returns: "Deal Boost: 10% for 7 Days"

// Category 2 examples
generateRewardName({ type: 'physical_gift', description: 'iPhone 16 Pro' })
// Returns: "Gift Drop: iPhone 16 Pro"

generateRewardName({ type: 'experience', description: 'Weekend Getaway' })
...
// Main reward schema (expanded from Loyalty.md:230-237)
export const CreateRewardSchema = z.object({
  ...
  if (category1.includes(data.type)) {
    if (!data.value_data) {
      ctx.addIssue({
        ...
        message: 'Category 1 rewards require value_data',
        path: ['value_data']
      });
      return;
    }
  ...
  if (category2.includes(data.type)) {
    if (!data.description || data.description.trim().length === 0) {
      ctx.addIssue({
        code: z.ZodIssueCode.custom,
        message: 'Category 2 rewards require description',
        path: ['description']
      });
    }
...
import { CreateRewardSchema } from '@/lib/schemas';
...
export async function POST(request: Request) {
  ...
  const body = await request.json();
  const validation = CreateRewardSchema.parse(body);

  // Auto-generate name
  const name = generateRewardName(validation);
...
export async function PUT(
  ...
  const name = generateRewardName(merged);
...
### Admin UI - Create Reward Form

```typescript
// components/admin/CreateRewardForm.tsx

import { useState } from 'react';
import { supabase } from '@/lib/supabase';
import { generateRewardName, CATEGORY_1_TYPES, CATEGORY_2_TYPES } from '@/lib/rewards';
import { toast } from '@/components/ui/toast';
...
export function CreateRewardForm() {
  ...
  const isCategory1 = CATEGORY_1_TYPES.includes(rewardType);
  const isCategory2 = CATEGORY_2_TYPES.includes(rewardType);

  // Generate live preview
  const previewName = rewardType
    ? generateRewardName({ type: rewardType, value_data: valueData, description })
    : '';
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6 max-w-2xl">
      ...
      {(rewardType === 'spark_ads' || rewardType === 'gift_card') && (
        <div>
      ...
      {isCategory2 && (
        <div>
          <label className="block text-sm font-medium mb-2">
            {rewardType === 'physical_gift' ? 'Gift Description' : 'Experience Description'}
          </label>
          <textarea
            value={description}
...
export function RewardCard({ reward }: RewardCardProps) {
  return (
    <div className="reward-card border rounded-lg p-4">
...
**Q1: What if admin wants to change reward type after creation?**
...
- **Solution:** Store `reward_id` in redemptions (not name), so historical data is preserved
...
- [ ] Auto-generated names match format per reward type
- [ ] Category 1 rewards require value_data (validation error if missing)
- [ ] Category 2 rewards require description (validation error if missing)
...
- [ ] Creator UI displays auto-generated reward names
- [ ] Invalid reward types throw error
...
- [ ] Reward type changes regenerate name correctly
- [ ] Database stores auto-generated name in rewards.name field

---

## 3.1 Smart Hybrid Storage

**Reference:** See Loyalty.md Section 3 (Reward Amounts) and lines 1572-1579 (rewards table value_data field)

**Decision:** Category 1 rewards use structured JSONB (`value_data`), Category 2 rewards use freeform TEXT (`description`).

**For validation schemas, see Pseudocode.md Section 2.1** (lines 1825-1924) which includes type-specific Zod validation for all reward types.
    ...
    <div className="reward-card border rounded-lg p-4">
      ...
      {reward.description && (
        <p className="text-sm text-gray-600 mb-4">
          {truncateDescription(reward.description, 150)}
        </p>
      )}

      <button
        onClick={() => showFullDescription(reward)}
...
### Master Create Reward Form (Conditional Rendering)
...
**Get all rewards with percent >10% (commission OR discount):**
```sql
SELECT * FROM rewards
...
ALTER TABLE rewards ADD CONSTRAINT check_category_data
CHECK (
...
**Rewards of structured data for fulfillment:**
...
- [ ] Category 1 rewards require value_data (validation error if missing)
- [ ] Category 2 rewards require description (validation error if missing)
- [ ] Admin UI shows correct form based on reward type
...
- [ ] Fulfillment UI shows correct task based on reward type
  ...
  return (
    <div className="border rounded-lg p-4 bg-white">
      ...
      <div className="mt-4 pt-4 border-t">
        ...
        <p className="text-sm font-medium">{mission.reward.name}</p>
      </div>
    </div>
  );
}
...
export async function claimMissionReward(
  ...
  const redemption = await db.redemptions.create({
    data: {
      user_id: userId,
      reward_id: progress.mission.reward_id,
      status: 'pending',
      tier_at_claim: user.current_tier,
      redemption_type: progress.mission.reward.redemption_type,
      claimed_at: new Date()
    }
  });
  ...
  await sendNotification(userId, {
    ...
    body: `Your ${progress.mission.reward.name} will be fulfilled within 24 hours. Check the Rewards tab for status.`
  });
...
export async function unlockNextMissionInSequence(redemptionId: string) {
  ...
  const fulfilledProgress = await db.mission_progress.findFirst({
    where: {
      ...
      mission: { reward_id: redemption.reward_id },
      status: 'claimed',
...
export async function GET(request: Request) {
  ...
  return completedMissions.map(progress => ({
    ...progress,
    display_name: MISSION_DISPLAY_NAMES[progress.mission.mission_type],
    reward_name: progress.mission.reward.name,
