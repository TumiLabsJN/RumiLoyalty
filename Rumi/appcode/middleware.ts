...
export async function middleware(request: NextRequest) {
  ...
  console.log(`[TIMING][Middleware] START for ${pathname}`);
  ...
  if (authToken) {
    ...
    console.log(`[TIMING][Middleware] setSession() for ${pathname}: ${Date.now() - t_setSession}ms`);
  ...
  if (!pathname.startsWith('/admin')) {
    console.log(`[TIMING][Middleware] TOTAL for ${pathname}: ${Date.now() - MW_START}ms`);
    return response;
  }
